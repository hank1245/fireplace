...
const FireParticle: React.FC<{ position: Vector3 }> = ({ position }) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      meshRef.current.position.x = position.x + Math.sin(time * waveSpeed) * waveAmplitude;
      meshRef.current.position.z = position.z + Math.cos(time * waveSpeed) * waveAmplitude;
...
export const Fireplace: React.FC = () => {
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      ...
      <mesh position={[0, 0.1, 0]} rotation={[0, Math.PI / 3, 0]} castShadow>
